  ...
  <main class="main item_more">
    <div class="control_set">
      ...
      <c:if test="${user.role.name() == 'JOB_SEEKER' && !vacancy.isClosed() && already_applied}">
        <form action="${pageContext.request.contextPath}/controller" method="get">
          ...
          <button class="btn"><fmt:message key="button.job_application"/></button>
        </form>
      </c:if>
      <c:if test="${user.role.name() == 'EMPLOYEE' && !vacancy.isClosed()}">
        <form action="${pageContext.request.contextPath}/controller" method="post">
          <input type="hidden" name="command" value="confirmation_page">
          <input type="hidden" name="target_command" value="vacancy_close">
          <input type="hidden" name="vacancy_id" value="${vacancy.id}">
          <button class="btn" style="background-color: var(--danger-color)"><fmt:message key="button.close"/></button>
        </form>
      </c:if>
      <c:if test="${user.role.name() == 'EMPLOYEE' && vacancy.isClosed()}">
        <form action="${pageContext.request.contextPath}/controller" method="post">
          <input type="hidden" name="command" value="confirmation_page">
          <input type="hidden" name="target_command" value="vacancy_remove">
          <input type="hidden" name="vacancy_id" value="${vacancy.id}">
          <button class="btn" style="background-color: var(--danger-color)"><fmt:message key="button.remove"/></button>
        </form>
      </c:if>
    </div>
    ...
    <div class="item_header">
      <h2 class="item_name">${vacancy.name}
        <c:if test="${vacancy.isClosed()}"><span style="color: var(--danger-color); font-size: 1rem;"><fmt:message key="messages.closed"/></span>
        </c:if>
      </h2>
    </div>
